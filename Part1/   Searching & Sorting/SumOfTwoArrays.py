    ...
    if n==0 and m ==0:
        ...
        if n==m:
            for i in range(0,n+1,1):
                x = arr1[n-i-1] + arr2[m-i-1] + carry_over
                output[n-i] = x%10
                carry_over = x//10
        elif n>m:
            i = 0
            ...
            for j in range(m-b,-1,-1):
                if j-1 >= 0:
                    ...
                    output[j] = 0 '''
    # for finding number from the given array
    #sum1,sum2,sum = 0,0,0
    #for i in range(0,n,1):
        #sum1 = arr1[i]*(10**(n-1)) + sum1
       # n = n - 1
    #for i in range(0,m,1):
       # sum2 = arr2[i]*(10**(m-1)) + sum2
       # m = m - 1
    #sum = sum1 + sum2
    #  for converting sum to int array
    #if n>m:
        #i = 0
        #while(sum>0):
            #r = sum%10
